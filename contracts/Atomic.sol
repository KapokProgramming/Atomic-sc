...
contract Atomic {
    ...
    function addVideo(
        ...
        addTags(video_id, _keywords);
    }

    function addTags(uint video_id, string[] calldata _keywords) internal {
        for (uint i = 0; i < _keywords.length; i++) {
            if (!term_video_idx_validation[_keywords[i]][video_id]) {
                term_video_idx[_keywords[i]].push(video_id);
                term_video_idx_validation[_keywords[i]][video_id] = true;
            }
        }
    }
    ...
    function getVideosByTerm(
        ...
        for (uint i = 0; i < terms.length; i++) {
            for (uint ii = 0; ii < term_video_idx[terms[i]].length; ii++) {
                videos[count++] = video_data[term_video_idx[terms[i]][ii]];
            }
        }
        return videos;
    }
